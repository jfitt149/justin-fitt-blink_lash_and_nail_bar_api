...
router.get("/:staffId/:serviceId", async (req, res, next) => {
  ...
  console.log(serviceId, serviceVersion, staffId);
  ...
  try {
    ...
    // res.json({ cancel, items: JSONBig.parse(JSONBig.stringify(items)) });

    res.json({
      availabilities: JSONBig.parse(JSONBig.stringify(availabilities)),
      serviceId,
      serviceVersion,
      ...JSONBig.parse(JSONBig.stringify(additionalInfo)),
    });
