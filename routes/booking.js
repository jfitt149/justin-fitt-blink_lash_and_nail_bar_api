...
router.post("/create", async (req, res, next) => {
  ...
  try {
    ...
    res.json(booking.id);
    // res.status(302).redirect(`/booking/${booking.id}`);
  } catch (error) {
    console.error(error);
    next(error);
  }
...
router.get("/:bookingId", async (req, res, next) => {
  ...
  try {
    ...
    res.json({
      // JSONBig.parse(JSONBig.stringify(availabilities))
      booking: JSONBig.parse(JSONBig.stringify(booking)),
      serviceItem: JSONBig.parse(JSONBig.stringify(serviceItem)),
      serviceVariation: JSONBig.parse(JSONBig.stringify(serviceVariation)),
      teamMemberBookingProfile: JSONBig.parse(
        JSONBig.stringify(teamMemberBookingProfile)
      ),
    });
